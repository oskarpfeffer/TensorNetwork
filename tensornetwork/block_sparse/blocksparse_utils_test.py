...
def test_compute_sparse_lookup():
  ...
  expected_unique = np.array([-11, -8, -7, -6, -4, -3, -2, -1, 0, 1, 2, 3, 5, 6, 9, 10])
...
def test_compute_num_nonzero(num_charges):
  ...
  nz2 = len(
    np.nonzero(
      np.logical_and.reduce(fused == np.zeros((1, num_charges), dtype=np.int16), axis=1)
    )[0]
  )
  assert nz1 == nz2
...
def test_find_diagonal_sparse_blocks(num_legs, num_charges):
  np.random.seed(10)
  np_charges = [
    np.random.randint(-5, 5, (60, num_charges), dtype=np.int16) for _ in range(num_legs)
  ]
  fused = np.stack(
    [
  ...
  # pylint: disable=no-member
  nz = np.nonzero(np.logical_and.reduce(fused == np.zeros((1, num_charges)), axis=1))[0]
  ...
  for n in range(unique_left.shape[0]):
    ...
    # pylint: disable=no-member
    blocks.append(linear_locs[np.nonzero(np.logical_and.reduce(left == ul, axis=1))[0]])
...
def test_find_transposed_diagonal_sparse_blocks(num_charges, order, D):
  ...
  np.random.seed(10)
  np_charges = [
    np.random.randint(-5, 5, (D, num_charges), dtype=np.int16) for _ in range(num_legs)
  ]
  tr_charge_list = []
  charge_list = []
  for c in range(num_charges):
    ...
    charge_list.append(fuse_ndarrays([np_charges[n][:, c] for n in range(num_legs)]))
