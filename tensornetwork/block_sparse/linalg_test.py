...
def test_create_diag(dtype, num_charges, flow):
  ...
  index = Index(
    BaseCharge(
      np.random.randint(-2, 3, (D, num_charges)), charge_types=[U1Charge] * num_charges
    ),
    flow,
  )
...
def test_diag_raises():
  ...
  indices = [
    Index(
      BaseCharge(np.random.randint(-2, 3, (Ds[n], 1)), charge_types=[U1Charge]), False
    )
    for n in range(rank)
  ]
...
def test_qr_r_mode():
  ...
  rank = len(Ds)
  charges = [
    BaseCharge(np.zeros((Ds[n], 1)), charge_types=[U1Charge] * 1) for n in range(rank)
  ]
  flows = [True] * rank
  A = BlockSparseTensor.random(
...
def test_eigh_prod(dtype, Ds, num_charges):
  ...
  A = BlockSparseTensor.random(inds + [i.copy().flip_flow() for i in inds], dtype=dtype)
  dims = np.prod(Ds)
