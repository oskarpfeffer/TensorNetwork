...
def get_charges(B0, B1, D, num_charges):
  return [np.random.randint(B0, B1 + 1, D).astype(np.int16) for _ in range(num_charges)]
...
def test_Charge_matmul(chargetype, B0, B1):
  ...
  Q = q1 @ q2 @ q3
  Q_ = BaseCharge(
    np.stack([C1, C2, C3], axis=1), charge_labels=None, charge_types=[chargetype] * 3
  )
...
def test_eq__raises():
  ...
  num_charges = 2
  charge = BaseCharge(
    np.random.randint(-2, 3, (30, num_charges)), charge_types=[U1Charge] * num_charges
  )
  with pytest.raises(ValueError):
...
def test_empty():
  num_charges = 4
  charges = BaseCharge(
    np.random.randint(-5, 6, (0, num_charges)), charge_types=[U1Charge] * num_charges
  )
...
def test_eq_raises():
  num_charges = 4
  c1 = BaseCharge(
    np.random.randint(-5, 6, (10, num_charges)), charge_types=[U1Charge] * num_charges
  )
  c2 = BaseCharge(
    np.random.randint(-5, 6, (0, num_charges)), charge_types=[U1Charge] * num_charges
  )
