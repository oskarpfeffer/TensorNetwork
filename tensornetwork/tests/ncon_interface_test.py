...
def test_node_order_spec(backend):
  ...
  result = ncon_interface.ncon(
    [node, node], [(-1, 1), (1, -2)], con_order=[1], out_order=[-1, -2], backend=backend
  )
  np.testing.assert_allclose(result.array, a @ a)

  result = ncon_interface.ncon(
    [node, node], [(-1, 1), (1, -2)], con_order=[1], out_order=[-2, -1], backend=backend
  )
...
def run_tests(a, b, c, backend):
  ...
  with pytest.raises(
    ValueError, match="number of indices does not match number of labels on tensor 0."
  ):
    ncon_interface.ncon([a, a], [(1,), (1, 2)], backend=backend)

  with pytest.raises(
    ValueError, match="only nonzero values are allowed to specify network structure."
  ):
  ...
  with pytest.raises(
    ...
    ncon_interface.ncon([a, a], [(1, 2), (2, 1)], con_order=[-1, 2], backend=backend)
  with pytest.raises(
    ValueError,
    match=r"all string type labels in `con_order` "
    r"must be unhyphenized, found \['-hi'\]",
  ):
    ncon_interface.ncon([a, a], [(1, 2), (2, 1)], con_order=["-hi", 2], backend=backend)
  ...
  with pytest.raises(
    ...
    ncon_interface.ncon([a, a], [(1, 2), (2, 1)], con_order=[3, 4, 5], backend=backend)
  ...
  with pytest.raises(
    ...
    ncon_interface.ncon([a, a], [(-1, 1), (1, -2)], out_order=[-1, 2], backend=backend)
  with pytest.raises(
    ValueError,
  ...
  with pytest.raises(
    ...
    ncon_interface.ncon([a, a], [(-1, 2), (2, -2)], out_order=[-3, -4], backend=backend)
