...
class PyTorchBackend(abstract_backend.AbstractBackend):
  ...
  def eye(self, N: int, dtype: Optional[Any] = None, M: Optional[int] = None) -> Tensor:
  ...
  def eigsh_lanczos(
    ...
    if not isinstance(initial_state, torchlib.Tensor):
      raise TypeError("Expected a `torch.Tensor`. Got {}".format(type(initial_state)))
  ...
  def trace(
    ...
    """
    if offset != 0:
      errstr = f"offset = {offset} must be 0 (the default)with PyTorch backend."
