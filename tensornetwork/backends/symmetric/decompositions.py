...
def svd(
  ...
  for n, b in enumerate(blocks):
    out = np.linalg.svd(
      np.reshape(matrix.data[b], shapes[:, n]), full_matrices=False, compute_uv=True
    )
  ...
  if (max_truncation_error is not None) or (max_singular_values is not None):
    ...
    if max_truncation_error is not None:
      if relative and (len(singvals) > 0):
        max_truncation_error = max_truncation_error * np.max([s[0] for s in singvals])
    ...
    if max_singular_values is not None:
      ...
      if max_singular_values < len(inds):
        discarded_inds = np.append(discarded_inds, inds[: (-1) * max_singular_values])
    ...
    discsingvals = [
      d[: (orig_block_size[n] - new_block_size[n])] for n, d in enumerate(discsingvals)
    ]
  ...
  if len(singvals) > 0:
    ...
    all_vblocks = np.concatenate(
      [np.ravel(v_blocks[n][0 : len(singvals[n]), :]) for n in range(len(v_blocks))]
    )
  else:
...
def rq(bt, tensor: BlockSparseTensor, pivot_axis: int) -> Tuple[Tensor, Tensor]:
  ...
  r, q = (
    bt.conj(bt.transpose(r, (1, 0))),
    bt.conj(bt.transpose(q, (1, 0))),
  )  # M=r*q at this point
