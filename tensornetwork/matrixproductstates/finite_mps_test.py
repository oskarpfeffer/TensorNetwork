...
def test_left_envs_two_non_consecutive_sites(backend_dtype_values):
  ...
  mps = FiniteMPS(
    tensors, center_position=None, backend=backend_dtype_values[0], canonicalize=False
  )
...
def test_left_envs_all_sites(backend_dtype_values):
  ...
  mps = FiniteMPS(
    tensors, center_position=N // 2, backend=backend_dtype_values[0], canonicalize=True
  )
  l = backend.convert_to_tensor(np.ones((1, 1), dtype=dtype))
  exp = {}
  for n, t in enumerate(mps.tensors):
...
def test_random_mps_invalid_dimensions_raises_error(backend_dtype_values):
  with pytest.raises(ValueError):
    FiniteMPS.random(
      d=[3, 4], D=[2, 3], dtype=backend_dtype_values[1], backend=backend_dtype_values[0]
    )
  with pytest.raises(ValueError):
    FiniteMPS.random(
      d=[3, 4, 4, 2],
