...
def test_BaseDMRG_init(backend_dtype_values):
  ...
  dmrg = BaseDMRG(
    mps, mpo, np.ones((1, 1, 1), dtype=dtype), np.ones((1, 1, 1), dtype=dtype), "name"
  )
...
def test_BaseDMRG_raises():
  ...
  with pytest.raises(
    TypeError,
    match="mps.dtype = {} is different from mpo.dtype = {}".format(
      mps.dtype, mpo.dtype
    ),
  ):
    BaseDMRG(
...
def test_finite_DMRG_init(backend_dtype_values, N):
  ...
  mpo = FiniteXXZ(
    Jz=np.ones(N - 1), Jxy=np.ones(N - 1), Bz=np.zeros(N), dtype=dtype, backend=backend
  )
  D = 32
  # test one-site DMRG
  mps = FiniteMPS.random([2] * N, [D] * (N - 1), dtype=dtype, backend=backend)
  ...
  mps = FiniteMPS.random([2] * N, [D] * (N - 1), dtype=dtype, backend=backend)
  dmrg = FiniteDMRG(mps, mpo)
  two_site_energy = dmrg.run_two_site(max_bond_dim=D, num_sweeps=4, num_krylov_vecs=10)
